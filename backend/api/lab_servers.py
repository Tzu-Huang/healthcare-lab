...
def create_lab_servers_blueprint(
    ...
    def lab_server_metadata():
        return jsonify({"success": True, **service.metadata()})

    @blueprint.get("/api/lab/servers")
    def list_lab_servers():
        return jsonify({"success": True, "items": service.list_servers()})

    @blueprint.post("/api/lab/servers")
    def create_lab_server():
        try:
            item = service.create_server(request.get_json(silent=True) or {})
    ...
    def get_lab_server(server_id: int):
        try:
            item = service.get_server(server_id)
        except KeyError:
    ...
    def update_lab_server(server_id: int):
        try:
            item = service.update_server(server_id, request.get_json(silent=True) or {})
        except KeyError:
            return error("Server was not found.", 404)
        except SimulatorValidationError as exc:
            return error(str(exc), 400)
    ...
    def check_lab_server(server_id: int):
        try:
            item = service.check_server(server_id)
        except KeyError:
            return error("Server was not found.", 404)
        except SimulatorValidationError as exc:
            return error(str(exc), 400)
    ...
    def smoke_all_lab_servers():
        return jsonify({"success": True, "items": service.smoke_all_servers()})
