...
class FhirInventoryService:
    ...
    def records(self, sync_status: str = "") -> list[dict[str, Any]]:
        return [item for item in self._repository.list_fhir_workflow_records(sync_status) if item["resourceType"] != "Task"]
...
class FhirPreviewService:
    ...
    def _fetch(self, reference: str, base_url: str) -> tuple[int, dict[str, Any]]:
        base = self._base_url_normalizer(base_url)
        return self._json_request(self._reference_url_builder(base, reference), "", auth_manager=self._auth_manager(), base_url=base)

    def resource_preview(self, reference: str, base_url: str = "") -> dict[str, Any]:
        resolved = str(base_url or self._medplum_base_url()).strip()
        if not resolved:
            raise ValueError("Medplum FHIR base URL is required.")
        status, resource = self._fetch(reference, resolved)
        return {"source": "medplum-live", "reference": reference, "statusCode": status, "resource": resource}
    ...
    def record_preview(self, record_id: int) -> dict[str, Any]:
        ...
        if (item.get("sync") or {}).get("status") == FHIR_SYNC_STATUS_SYNCED and reference:
            try:
                status, resource = self._fetch(reference, self._medplum_base_url())
                return {"item": item, "resource": resource, "source": "medplum-live", "live": {"fetched": True, "statusCode": status, "reference": reference, "error": ""}}
            except (ValidationError, SimulatorValidationError, UpstreamFhirError) as exc:
                return {"item": item, "resource": fallback, "source": "local-submitted-fallback", "live": {"fetched": False, "statusCode": self._upstream_status(str(exc)), "reference": reference, "error": str(exc)}}
...
class FhirWorkflowService:
    ...
    def mappings(self) -> list[dict[str, Any]]:
        return self.inventory_service.mappings()

    def records(self, sync_status: str = "") -> list[dict[str, Any]]:
        return self.inventory_service.records(sync_status)

    def inventory(self, sync_status: str = "", resource_type: str = "") -> dict[str, Any]:
        return self.inventory_service.inventory(sync_status, resource_type)
