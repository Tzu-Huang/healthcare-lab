...
Healthcare Lab MUST provide no force, wildcard, bulk, skip-preview, automatic-adoption, or revision-override path for managed Channel mutations, and SHALL revalidate ownership and live state immediately before every mutation.
...
- **WHEN** Healthcare Lab starts or reloads configuration
...
Healthcare Lab SHALL deploy or undeploy only one explicitly identified managed Channel after refreshed ownership validation and SHALL NOT expose the OIE redeploy-all primitive.
...
- **WHEN** a deploy or undeploy request resolves to an external or conflicted Channel
...
Healthcare Lab SHALL delete only one explicitly confirmed exact managed Channel, SHALL undeploy it first when required, and SHALL retain its logical template mapping while clearing the deleted OIE Channel ID and revision.

#### Scenario: Managed Channel is deleted
- **WHEN** a valid delete preview and matching destructive confirmation remain current
