...
The `HLAB_ORU_TO_HLAB` destination SHALL enable OIE queueing, retry indefinitely at 10-second intervals, retain a queue buffer of 1000, queue response timeouts, and use 5000 ms send and response timeouts.

#### Scenario: Render resilient ORU delivery settings

- **WHEN** the ORU template is compiled
- **THEN** its destination queue is enabled with the required retry, buffer, and timeout values
