...
- [x] 2.1 Persist Save-and-test outcomes only when they still match the effective Medplum configuration revision.
- [x] 2.2 Derive Medplum readiness as ready, degraded, or needs-setup from matching successful, matching failed, or missing/stale bounded evidence.
- [x] 2.3 Ensure explicit re-verification can replace degraded evidence after recovery without adding startup or readiness-read network calls.
