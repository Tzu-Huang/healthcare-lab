...
- [x] 2.4 Audit new response and error paths so the password and request body are never logged or serialized.

## 3. Backend API

- [x] 3.1 Add `GET /api/oie/settings` to return the persisted secret-safe profile.
- [x] 3.2 Add `PUT /api/oie/settings` to validate and atomically save the full profile and mapping collection.
- [x] 3.3 Keep persisted listener settings separate from current listener start, stop, status, and app-start behavior.
...
- [x] 4.2 Add API tests for reads, valid updates, actionable URL/host/timeout/port errors, and atomic rejection.
- [x] 4.3 Add secret-safety tests covering default masking, password preservation/replacement, empty-password rejection, and response/log exclusion.
...
- [x] 4.5 Run the focused Python test suite and strict OpenSpec validation.
