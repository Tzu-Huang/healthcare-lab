...
- [ ] 1.2 Create or identify a versioned canonical pre-unified-Settings database fixture and document its schema provenance.
- [ ] 1.3 Add secret and PHI canaries plus bounded evidence-capture helpers that reject unsafe output.

## 2. Fresh-Install Verification

- [ ] 2.1 Add automated coverage for startup with no `.env`, database, or prior volumes and for accurate initial readiness/default projections.
...
- [ ] 3.1 Add the canonical legacy database and representative eligible environment values to an isolated upgrade harness.
...
- [ ] 3.3 Change migrated settings through the UI, restart with conflicting legacy environment values, and prove persisted authority and bootstrap idempotence.
...
- [ ] 4.5 Add or confirm contracts proving Settings cannot rewrite Compose or invoke arbitrary Docker operations.
