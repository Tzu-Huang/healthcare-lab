...
- [x] 4.2 Verify clean Compose startup creates and starts both routes, ordinary restart performs no mutation, and partial setup creates only the absent member using bounded secret-safe evidence.
- [x] 4.3 Run focused bootstrap/settings/lifecycle tests, the complete regression suite, Python compilation, `git diff --check`, and strict OpenSpec validation.
