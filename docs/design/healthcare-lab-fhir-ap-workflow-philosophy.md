...
FHIR resources are the visual language. Labels should name the resource families, not explain the whole implementation. ServiceRequest forms the order current; DiagnosticReport, Observation, DocumentReference, and Binary form the result current. The diagram must be meticulously crafted so those two currents are distinct at presentation distance.
