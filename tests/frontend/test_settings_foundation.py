...
class SettingsFoundationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.oie_view = (ROOT / "frontend/static/js/settings/oie.js").read_text(encoding="utf-8")
    ...
    def test_connection_is_write_only_and_tested_from_saved_settings(self):
        ...
        self.assertIn("if (replacement) payload.password = replacement", self.oie_view)
        self.assertIn("test-connection", self.api)
        self.assertNotIn("password = management.password", self.oie_view)
    ...
    def test_listener_intent_runtime_and_warning_are_separate(self):
        ...
        self.assertIn("!listenerSettingsMatchStatus(state.profile, runtime.item)", self.oie_view)
    ...
    def test_layered_diagnostics_are_safe_and_distinguish_recovery_actions(self):
        ...
        self.assertIn("refreshRuntimeDiagnostics", self.oie_view)
        for phrase in ("Apply/Redeploy", "Retry or lab-app restart", "container recreation"):
            self.assertIn(phrase, self.template)
        self.assertIn('check.state || "unknown"', self.oie_view)
        self.assertIn('return `Queued: ${evidence.queued}; Errors: ${evidence.errors}`', self.oie_view)
        self.assertIn('return "Counts unavailable."', self.oie_view)
        self.assertIn('check.layer !== "delivery-state"', self.oie_view)

    def test_preview_is_required_and_delete_matches_display_name(self):
        self.assertIn("state.preview?.previewToken", self.oie_view)
        self.assertIn("state.confirmation !== expectedName", self.oie_view)
        self.assertIn("displayedChannelName(selectedItem(), state.preview)", self.oie_view)
        self.assertIn("recreate", self.api)
        combined = (self.api + self.oie_view + self.template).lower()
    ...
    def test_external_cards_have_no_mutation_controls(self):
        ...
        self.assertIn(external_guard, self.oie_view)
        self.assertLess(self.oie_view.index(external_guard), self.oie_view.index("normalizedActions(item).forEach"))
    ...
    def test_editor_exposes_only_bounded_owned_fields(self):
        for field in ("sourceHost", "sourcePort", "destinationHost", "destinationPort", "timeoutSeconds", "queueEnabled", "retryCount", "retryIntervalMs"):
            self.assertIn(field, self.oie_view)
        for forbidden in ("raw xml", "raw json", "transformer", "filter", "script editor"):
            self.assertNotIn(forbidden, (self.oie_view + self.template).lower())
        self.assertIn("Preview Apply before changing OIE", self.oie_view)
